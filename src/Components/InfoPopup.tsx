...
export const InfoPopup = ({ children, show, x, y }: PropsWithChildren<PopupProps>) => {
  ...
  return show ? <div style={{ position: "fixed", zIndex: 1, left, top }}>
    ...
    <ShadowBox boxShadow="2xl" borderWidth="3.5px"
               borderColor={FormSubmitBg} // TODO: Change border color to something better
...
export const InfoButton = (props: PropsWithChildren<ButtonProps>) => {
  ...
  return <Box position="relative" display="grid" {...props}>
    <div style={{ display: "inline-block", justifySelf: "center" }} onMouseEnter={setFlag.on} onMouseLeave={setFlag.off}
         onMouseMove={positionExtract}>
      <Icon color="black" />
    </div>
